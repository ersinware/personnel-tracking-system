<script>
    ...
    import {EVENT_UNIT_REMOVED} from "$lib/js/client/events.client.js";
    ...
    async function onRemoveClick(event) {
        ...
        _fetch(
            '/api/unit/remove-unit',
            {method: 'DELETE', body: unitId},
            async () => {
                state = 'navigated'
                ...
                window.dispatchEvent(new CustomEvent(EVENT_UNIT_REMOVED))
            },
            async (body) => {
                state = 'navigated'
                await waitFor(DURATION_TRANSITION_MODAL_PROGRESS_BAR_NAVIGATED)
...
<article class="wrapper-modal-content flex f-column a-i-c g-v-d-modal big-screen-max-w-modal-content-small">
    ...
    <p class="text-modal">
        '{unitName}' isimli birim ve birime bağlı olan çalışmalar ile kullanıcılar silinecek.

        <br>
        <br>

        <span class="color-text-error f-w-500">
            Bu işlem geri alınamaz. Birimi gerçekten kalıcı olarak silmek istiyor musunuz?
        </span>
    </p>
