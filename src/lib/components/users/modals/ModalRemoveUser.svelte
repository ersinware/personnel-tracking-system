<script>
    ...
    import {EVENT_USER_REMOVED} from "$lib/js/client/events.client.js";
    ...
    async function onRemoveClick(event) {
        ...
        _fetch(
            ...
            async () => {
                ...
                window.dispatchEvent(new CustomEvent(EVENT_USER_REMOVED))
            },
            async (body) => {
                state = 'navigated'
                await waitFor(DURATION_TRANSITION_MODAL_PROGRESS_BAR_NAVIGATED)
...
<article class="wrapper-modal-content flex f-column a-i-c g-v-d-modal big-screen-max-w-modal-content-small">
    ...
    <p class="text-modal">
        '{user.fullName}' isimli kullanıcı ve çalışmaları silinecek.

        <br>
        <br>

        <span class="color-text-error f-w-500">
            Bu işlem geri alınamaz. Kullanıcıyı gerçekten kalıcı olarak silmek istiyor musunuz?
        </span>
    </p>
